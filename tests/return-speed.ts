...
function step(state: HockeyGameState, count = 1): HockeyGameState { let next = state; for (let index = 0; index < count; index += 1) next = simulation.stepGame(next, EMPTY_ACTION_SNAPSHOT, FIXED_STEP_SECONDS); return next; }
...
assert.ok(wall.puck.velocity.x > 900 * WALL_RESTITUTION, `wall restitution does not reapply the return multiplier: ${wall.puck.velocity.x}`);
