...
function syncButtons(){document.querySelectorAll('button').forEach(b=>b.setAttribute('aria-pressed',String(b.textContent===state||b.textContent===(active==='player1'?'Player 1':'Player 2'))))}function choose(id,age){const winner=id===active;if(state==='Idle')return Math.floor(age/100)%25===24?1:0;if(state==='Goal')return winner?(reduced?5:age<100?2:age<250?3:age<520?4:5):(reduced?8:age<140?6:age<480?7:8);if(state==='Conceded')return winner?(reduced?8:age<140?6:age<480?7:8):0;if(state==='Win')return winner?(reduced?14:age<140?9:age<280?10:age<460?11:age<680?12:age<920?13:14):15;if(state==='Defeated')return winner?15:0;return 0}function render(now){const age=now-start;for(const id of Object.keys(cats)){const frame=choose(id,age);const col=frame%${descriptor.sheet.columns},row=Math.floor(frame/${descriptor.sheet.columns});cats[id].style.backgroundImage='url('+images[id]+')';cats[id].style.backgroundPosition=(-col*80)+'px '+(-row*80)+'px';document.querySelector('[data-status-'+id+']').textContent=(id==='player1'?'P1 ':'P2 ')+state+' · frame '+frame}requestAnimationFrame(render)}syncButtons();requestAnimationFrame(render);
</script></body></html>\n`;
}
