...
The default art direction is a cute pixel-art cat rink with Godot-authored cat-paw strikers, a circular yarn puck, couch goals, and persistent impact effects. The yarn puck adopts the last hitter's score-cat palette; each authoritative paw contact synchronizes seven authored puck and contacting-paw frames over 0.64 seconds, plus a strength-scaled directional recoil derived from the collision normal. Ready, countdown, pause, goal, won, and reduced-motion presentation hold frame 0; reduced motion still preserves palette ownership. The under-couch opening is the scoring mouth. The static full-surface Board is a replaceable rectangular bitmap below the dynamic goals, cats, paws, puck, HUD, and effects. Static custom theme cells retain the transform-based contact fallback; all user artwork stays local and adds no runtime requests.
