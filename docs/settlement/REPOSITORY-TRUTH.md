...
- The direct materialized command passed, while SFHS's spawned `pnpm run test` failed when its CLI was itself launched by pnpm: the inherited package lifecycle context resolved generic checks against the SFHS workspace. The product `test` script now runs its same five Node test commands directly, and Quality launches the pinned CLI with Node by absolute path so its spawned commands resolve in the materialized project. Quality runs the materialized `pnpm test` first with visible output and retains SFHS check as a separate required gate.
