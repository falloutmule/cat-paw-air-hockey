...
- **INFERRED:** invoking the CLI as `pnpm sfhs` lets its lifecycle environment leak into its child `pnpm` commands, obscuring the materialized-package command context. Quality now invokes the pinned CLI directly through `pnpm exec node packages/cli/src/main.ts`, runs the materialized `pnpm test` first with visible output, and retains SFHS check as a separate required gate.
